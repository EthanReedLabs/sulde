...
Sulde 的核心通过协议和宿主能力契约接入 Agent 工具。本文记录当前已提供的 Claude Code
与 Codex 两个适配器；兼容工具可复用相同接口，其他完整宿主接入需补齐适配并验证，见
[扩展兼容宿主](DEVELOPMENT.zh-CN.md#扩展兼容宿主)。

两个现有适配器共享同一套核心状态，支持以下运行方式：
